...
/*
  double _userHeading = 0; // الاتجاه بالدرجات
    ...
    controller.animateCamera(
      CameraUpdate.newCameraPosition(
        CameraPosition(
          ...
          zoom: 17.5,
          bearing: _userHeading, // الاتجاه هنا
        ),
      ),
    );
  }
  ...
  Future<void> _initLocation() async {
    ...
    // تحديث الكاميرا للمرة الأولى
    _updateCamera();

    // الاستماع لتغير موقع اليوزر
    _positionStream =
        Geolocator.getPositionStream(
          ...
          _updateCamera(); // تحديث الكاميرا مع الموقع الجديد
        });

    // الاستماع لاتجاه الجهاز
    _headingStream = FlutterCompass.events?.listen((CompassEvent event) {
      setState(() {
        _userHeading = event.heading ?? 0;
      });
      if (_userPosition != null) {
        _updateCamera(); // تحديث الكاميرا مع الاتجاه الجدي
        */
